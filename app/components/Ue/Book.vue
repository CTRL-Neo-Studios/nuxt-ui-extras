...
const perspectiveClasses = computed(() => {
	return twMerge("book-perspective w-[196px]", ui.value.perspective, props.class);
});
...
async function toLink() {
	if (props.to) await navigateTo(props.to, { external: props.external });
}
</script>
		...
		<div :class="wrapperClasses" :style="wrapperStyle" data-slot="wrapper">
			...
			<div :class="slotClass('book-cover', 'cover')" data-slot="cover">
				...
				<template v-if="variant === 'stripe'">
					...
					<div
						:class="slotClass('book-stripe-bottom', 'stripeBottom')"
						data-slot="stripe-bottom"
					>
						<span :class="slotClass('book-title', 'title')" data-slot="title">{{
							title
						}}</span>
			...
			<!-- BACK COVER -->
			<div :class="slotClass('book-back', 'back')" aria-hidden="true" data-slot="back" />
		</div>
	</div>
</template>
